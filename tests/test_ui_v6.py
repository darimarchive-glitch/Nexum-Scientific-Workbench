...
class UIV6(unittest.TestCase):
    def test_calculator_sidebar_is_compact_and_collapsible(self):
        s=(ROOT/'nexum/ui/calculators_page.py').read_text()
    ...
    def test_calculator_has_native_plot(self):
        s=(ROOT/'nexum/ui/calculators_page.py').read_text();self.assertIn('ScientificPlot',s)
    def test_experiment_sidebars_are_compact(self):
        s=(ROOT/'nexum/ui/experiments_page.py').read_text();self.assertIn('set_size_request(244, -1)',s);self.assertIn('set_size_request(286, -1)',s)
    ...
    def test_home_workspace_exists(self):
        s=(ROOT/'nexum/ui/main_window.py').read_text();self.assertIn('HomePage',s);self.assertIn('"home","Início"',s)
